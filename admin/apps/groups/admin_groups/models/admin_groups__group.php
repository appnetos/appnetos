...
// Model "admin\groups\admin_groups__group".
class admin_groups__group
{
    ...
    public function init()
    {
        ...
        $groupsUris = objects::get('admin/groups/admin_groups__uris');

        // Select from database table "admin_groups".
        $database = objects::get('database');
        $query = 'SELECT * FROM admin_groups WHERE xt_id=?';
        ...
        if (trim($rowGranted) !== '') {
            ...
            foreach ($granteds as $granted) {
                ...
                if ($uri !== false) {
                    if ($uri === '') {
                        $this->granted[$granted] = $strings->get('admin__groups__admin_groups__home');
                    }
                    else {
        ...
        if (trim($rowDenied) !== '') {
            ...
            foreach ($denieds as $denied) {
                ...
                if ($uri !== false) {
                    if ($uri === '') {
                        $this->denied[$denied] = $strings->get('admin__groups__admin_groups__home');
                    }
                    else {
    ...
    protected function restore()
    {
        ...
        $query = "UPDATE admin_groups SET xt_granted=?, xt_denied=? WHERE xt_id=?";
    ...
    public function edit()
    {
        ...
        // Update database table "admin_groups".
        $database = objects::get('database');
        $query = 'UPDATE admin_groups SET xt_name=? WHERE xt_id=?';
    ...
    public function addGranted()
    {
        ...
        // If no URIS to add.
        if (!count($add)) {
            $this->render('admin__groups__admin_groups__no_uris_err');
        }
        ...
        // Update database table "admin_groups".
        $database = objects::get('database');
        $query = 'UPDATE admin_groups SET xt_granted=? WHERE xt_id=?';
        ...
        foreach ($granted as $value) {
            ...
            if ($uri === '') {
                $this->granted[$value] = $strings->get('admin__groups__admin_groups__home');
            }
            else {
        ...
        // Render.
        $this->render(null, 'admin__groups__admin_groups__add_uri_conf');
    }
    ...
    public function addDenied()
    {
        ...
        // Update database table "admin_groups".
        $database = objects::get('database');
        $query = 'UPDATE admin_groups SET xt_denied=? WHERE xt_id=?';
        ...
        foreach ($denied as $value) {
            $uri = $groupsUris->get($value);
            if ($uri === '') {
                $this->denied[$value] = $strings->get('admin__groups__admin_groups__home');
            }
            else {
    ...
    public function removeGranted()
    {
        ...
        // Render.
        $this->render(null, 'admin__groups__admin_groups__remove_uri_conf');
    }
    ...
    protected function render($error = null, $confirm = null)
    {
        ...
        $render->assign('admin__groups__admin_groups__group', $this);
        $strings = objects::get('strings');
        if ($error) {
        ...
        $groupsSearch->init();
        $output = $render->fetch('admin/apps/groups/admin_groups/views/admin_groups__group.tpl');
        echo $output;
        exit();
    }
