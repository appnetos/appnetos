...
// Model "admin\apps\picker_application__groups_list".
class picker_application__groups_list
{
    ...
    public function init()
    {
        ...
        $render->assign('admin__groups__picker_application__groups_list', $this);

        // Get model objects.
        $database = objects::get('database');
        $pickerSearch = objects::get('admin/groups/picker_application__search');
        ...
        // Select data from database table "application_groups".
        $query = 'SELECT xt_id FROM application_groups WHERE xt_name LIKE ? ORDER BY ' . $pickerSearch->order .' LIMIT ' . $pickerSearch->number . ' OFFSET ' . $offset;
        ...
        // Get object "admin\groups\picker_application__group".
        objects::get('admin/groups/picker_application__group');

        // Initialize apps.
        for ($i = 0; $i < count($array); $i++) {
            $pickerGroup = objects::getNew('admin/groups/picker_application__group');
