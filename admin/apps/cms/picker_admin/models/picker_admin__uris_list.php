...
// Controller "admin\cms\picker_admin__uris_list".
class picker_admin__uris_list
{
    ...
    public function init()
    {
        ...
        $render->assign('admin__cms__picker_admin__uris_list', $this);
        ...
        $query = 'SELECT COUNT(*) FROM admin_cms WHERE (xt_uri LIKE ? OR xt_title LIKE ?) ' . $excluded . 'AND xt_language_key=?';
        ...
        $query = 'SELECT xt_id, xt_uri FROM admin_cms WHERE (xt_uri LIKE ? OR xt_title LIKE ?) ' . $excluded . 'AND xt_language_key=? ORDER BY ' . $pickerSearch->order .' LIMIT ' . $pickerSearch->number . ' OFFSET ' . $offset;
        ...
        // Get URIs as model "admin\general\picker_admin__uri".
        objects::get('admin\cms\picker_admin__uri');

        // Initialize URIs.
        for ($i = 0; $i < count($array); $i++) {
            $uri = objects::getNew('admin/cms/picker_admin__uri');
