...
// Model "admin\users\admin_management__user".
class admin_management__user
{
    ...
    public function init()
    {
        ...
        $query = null;
        $query = 'SELECT * FROM admin_users WHERE xt_id=?';
    ...
    public function edit()
    {
        ...
        if (!$deleteImage) {
            if (isset($_FILES['image'])) {
                if (isset($_FILES['image']['name']) && isset($_FILES['image']['tmp_name']) && isset($_FILES['image']['size'])) {
        ...
        // If user not exists.
        if ($this->error) {
            $this->render('admin__users__admin_management__edit_err');
        }
        ...
        if ($image) {
            if (!$account->verifyImageType($image)) {
                $this->render('admin__users__admin_management__edit_err_img_type');
            }
            if (!$account->verifyImageSize($image)) {
                $this->render('admin__users__admin_management__edit_err_img_size');
            }
        }
        ...
        if ($user !== $this->user) {
            if (!$account->verifyUserExists($user)) {
                $this->render('admin__users__admin_management__edit_err_user_exists');
            }
            if ($minUserVerify) {
                $account->minUserVerify = true;
            } else {
                if (!$account->verifyUserValid($user)) {
                    $this->render('admin__users__admin_management__edit_err_user_valid');
                }
                if (!$account->verifyUserUsable($user)) {
                    $this->render('admin__users__admin_management__edit_err_user_usable');
                }
            }
        }
        ...
        if ($mail !== $this->mail) {
            if (!$account->verifyMailExists($mail)) {
                $this->render('admin__users__admin_management__edit_err_mail_exists');
            }
            if (!$account->verifyMailValid($mail)) {
                $this->render('admin__users__admin_management__edit_err_mail_valid');
            }
        }
        ...
        if ($pass) if ($pass !== '') {
            if ($minPassVerify) {
                ...
                if (!$account->verifyPassValid($pass)) {
                    $this->render('admin__users__admin_management__edit_err_pass_valid');
                }
                if (!$account->verifyPassUsable($pass)) {
                    $this->render('admin__users__admin_management__edit_err_pass_usable');
                }
            }
        ...
        // Render.
        $this->init();
        $this->render(null, 'admin__users__admin_management__edit_conf');
    }
    ...
    public function lock()
    {
        // Get parameters.
        $post = objects::get('post');
        $id = $post->get('admin__users__admin_management__parameters');
        $id = (int)$id;
        ...
        // If user not exists.
        if ($this->error) {
            $this->render('admin__users__admin_management__err_lock');
        }
        ...
        // Render.
        $this->init();
        $this->render(null, 'admin__users__admin_management__conf_lock');
    }
    ...
    public function activate()
    {
        ...
        // If user not exists.
        if ($this->error) {
            $this->render('admin__users__admin_management__err_activate');
        }
        ...
        // Render.
        $this->init();
        $this->render(null, 'admin__users__admin_management__conf_activate');
    }
    ...
    protected function render($error = null, $confirm = null)
    {
        ...
        $render->assign('admin__users__admin_management__user', $this);
        ...
        $output = $render->fetch('admin/apps/users/admin_management/views/admin_management__user.tpl');
        echo $output;
        exit();
    }
}