...
// Model "admin\users\admin_management__users_list".
class admin_management__users_list
{
    ...
    public function init()
    {
        ...
        $render->assign('admin__users__admin_management__users_list', $this);

        // Get used objects.
        $database = objects::get('database');
        $managementSearch = objects::get('admin/users/admin_management__search');

        // Set selection.
        $selection = '';
        if ($managementSearch->selection === 'active') {
        ...
        // Select count from database table "admin_users".
        $query = 'SELECT COUNT(*) FROM admin_users WHERE (xt_user LIKE ? OR xt_mail LIKE ?)' . $selection;
        ...
        $query = 'SELECT xt_id FROM admin_users WHERE (xt_user LIKE ? OR xt_mail LIKE ?)' . $selection . ' ORDER BY ' . $managementSearch->order .' LIMIT ' . $managementSearch->number . ' OFFSET ' . $offset;
        ...
        // Get object "admin\users\admin_management__user".
        objects::get('admin/users/admin_management__user');

        // Initialize apps.
        for ($i = 0; $i < count($array); $i++) {
            $managementUser = objects::getNew('admin/users/admin_management__user');
    ...
    public function add()
    {
        ...
        $parameters = $post->get('admin__users__admin_management__parameters');

        // If parameters wrong.
        if (gettype($parameters) !== 'array') {
            $this->render('admin__users__admin_management__add_err');
        }
        ...
        if ($user === '') {
            $this->render('admin__users__admin_management__add_err_user_enter');
        }
        if ($pass === '') {
            $this->render('admin__users__admin_management__add_err_pass_enter');
        }
        if ($mail === '') {
            $this->render('admin__users__admin_management__add_err_mail_enter');
        }
        ...
        if (!$account->verifyUserExists($user)) {
            $this->render('admin__users__admin_management__add_err_user_exists');
        }
        if ($minUserVerify) {
            $account->minUserVerify = true;
        } else {
            if (!$account->verifyUserValid($user)) {
                $this->render('admin__users__admin_management__add_err_user_valid');
            }
            if (!$account->verifyUserUsable($user)) {
                $this->render('admin__users__admin_management__add_err_user_usable');
            }
        }

        // Verify mail address.
        if (!$account->verifyMailExists($mail)) {
            $this->render('admin__users__admin_management__add_err_mail_exists');
        }
        if (!$account->verifyMailValid($mail)) {
            $this->render('admin__users__admin_management__add_err_mail_valid');
        }
        ...
        if ($minPassVerify) {
            ...
            if (!$account->verifyPassValid($pass)) {
                $this->render('admin__users__admin_management__add_err_pass_valid');
            }
            if (!$account->verifyPassUsable($pass)) {
                $this->render('admin__users__admin_management__add_err_pass_usable');
            }
        }
        ...
        // Render.
        $this->render(null, 'admin__users__admin_management__add_conf');
    }
    ...
    public function delete()
    {
        ...
        // If parameters wrong.
        if (gettype($parameters) !== 'array') {
            $this->render('admin__users__admin_management__err_delete');
        }
        ...
        // Render.
        $this->render(null, 'admin__users__admin_management__conf_delete');
    }
    ...
    protected function render($error = null, $confirm = null)
    {
        ...
        // Get object "admin\users\admin_management__model".
        $managementModel = objects::get('admin\users\admin_management__model');
        $managementModel->init();

        // Render template.
        $output = $render->fetch('admin/apps/users/admin_management/views/admin_management__users_list.tpl');
        echo $output;
        exit();
    }
}