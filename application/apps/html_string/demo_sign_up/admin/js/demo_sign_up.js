...
*/
 ...
 // JavaScript.
var appnetos__html_string__demo_sign_up__textareas = [];
var appnetos__html_string__demo_sign_up__instances = [];
var appnetos__html_string__demo_sign_up__css = null;
$(document).ready(function() {
    if ($('form[data-html="true"]').attr('data-html') === 'true') {
        $('input[type=text], textarea').each(function () {
            appnetos__html_string__demo_sign_up__textareas.push($(this).attr('id'));
            var tmp = new nicEditor({fullPanel: true}).panelInstance($(this).attr('id'));
            appnetos__html_string__demo_sign_up__instances.push(tmp);
        });
    }
    else {
        $('input[type=text], textarea').each(function () {
    ...
    if (typeof appnetos__html_string__css != null) {
        appnetos__html_string__demo_sign_up__css = CodeMirror(function (elt) {
...
function appnetos__html_string__demo_sign_up__demo_sign_up__edit() {
    ...
    for (var i = 0; i < appnetos__html_string__demo_sign_up__instances.length; i++) {
        var input = document.createElement('input');
        input.setAttribute('type', 'hidden');
        input.setAttribute('name', appnetos__html_string__demo_sign_up__textareas[i]);
        input.setAttribute('value', appnetos__html_string__demo_sign_up__instances[i].getValue());
        form.appendChild(input);
    }
    var input = document.createElement('input');
    input.setAttribute('type', 'hidden');
    input.setAttribute('name', 'appnetos__html_string__css');
    input.setAttribute('value', appnetos__html_string__demo_sign_up__css.getValue());
    form.appendChild(input);
