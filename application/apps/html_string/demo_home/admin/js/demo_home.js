...
*/
 ...
 // JavaScript.
var appnetos__html_string__demo_home__textareas = [];
var appnetos__html_string__demo_home__instances = [];
var appnetos__html_string__demo_home__css = null;
$(document).ready(function() {
    if ($('form[data-html="true"]').attr('data-html') === 'true') {
        $('input[type=text], textarea').each(function () {
            appnetos__html_string__demo_home__textareas.push($(this).attr('id'));
            var tmp = new nicEditor({fullPanel: true}).panelInstance($(this).attr('id'));
            appnetos__html_string__demo_home__instances.push(tmp);
        });
    }
    else {
        $('input[type=text], textarea').each(function () {
    ...
    if (typeof appnetos__html_string__css != null) {
        appnetos__html_string__demo_home__css = CodeMirror(function (elt) {
...
function appnetos__html_string__demo_home__demo_home__edit() {
    ...
    for (var i = 0; i < appnetos__html_string__demo_home__instances.length; i++) {
        var input = document.createElement('input');
        input.setAttribute('type', 'hidden');
        input.setAttribute('name', appnetos__html_string__demo_home__textareas[i]);
        input.setAttribute('value', appnetos__html_string__demo_home__instances[i].getValue());
        form.appendChild(input);
    }
    var input = document.createElement('input');
    input.setAttribute('type', 'hidden');
    input.setAttribute('name', 'appnetos__html_string__css');
    input.setAttribute('value', appnetos__html_string__demo_home__css.getValue());
    form.appendChild(input);
