...
function install()
{
    ...
    $array = $database->selectArray($query, ['appnetos\widgets', 'appnetos', 'Widget Remove Demo']);
    ...
    $install->name = 'Widget Remove Demo';
    $install->description = 'APPNET OS Widget Remove Demo ' . $date->format('Y.m.d H:i:s');
    $install->widget = 1;
    $install->install();
    ...
    if (!file_exists('out/admin/img/appnetos/widget_remove_demo.svg')) {
        copy('application/apps/appnetos/widget_remove_demo/admin/files/widget_remove_demo.svg', 'out/admin/img/appnetos/widget_remove_demo.svg');
    }
}