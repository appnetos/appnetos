...
function install()
{
    ...
    $array = $database->select($query, ['appnetos\widgets', 'appnetos', 'Widget Apps']);
    ...
    $install->name = 'Widget Apps';
    $install->description = 'APPNET OS Widget Apps ' . $date->format('Y.m.d H:i:s');
    $install->widget = 1;
    $install->install();
    ...
    // Copy files.
    if (!file_exists('out/admin/img/appnetos/widget_apps.svg')) {
        copy('application/apps/appnetos/widget_apps/admin/files/widget_apps.svg', 'out/admin/img/appnetos/widget_apps.svg');
    }
}